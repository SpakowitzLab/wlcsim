...
use params, only: wlc_V, wlc_R, wlc_RP, wlc_AB, wlc_U&
    ...
    wlc_nucleosomeWrap, wlc_basepairs
...
DR = nint(exponential_random_int(window, rand_stat)*(urand(1)-0.5))
if (DR == 0) then 
    return
endif
...
outer: do II = 1, I-(prevNuc(1)+1) ! explore the previous linker space
    inner: do JJ = 1, (nextNuc(1)-1)-I ! explore the next linker space
        if ((wlc_basepairs(I-II) + DR > 1) .AND. (wlc_basepairs(I-II) + DR < 3*(sum(wlc_basepairs)/WLC_P__NB)) .AND. &
        (wlc_basepairs(I+JJ) - DR > 1) .AND. (wlc_basepairs(I+JJ) - DR < 3*(sum(wlc_basepairs)/WLC_P__NB)) ) then
...
if (success) then
    ...
    do KK = IT1, IT2
        if ( I == KK .AND. DR > 0) then
            wlc_RP(:,KK) = wlc_R(:,KK) + wlc_U(:,KK-1)*DR*WLC_P__LENGTH_PER_BP
        else if ( I == KK .AND. DR < 0) then
            wlc_RP(:,KK) = wlc_R(:,KK) + wlc_U(:,KK+1)*DR*WLC_P__LENGTH_PER_BP
        else 
            wlc_RP(:,KK) = wlc_R(:,KK) + wlc_U(:,KK)*DR*WLC_P__LENGTH_PER_BP
        endif
        wlc_UP(:,KK) = wlc_U(:,KK)
            if (WLC_P__LOCAL_TWIST) wlc_VP(:,KK) = wlc_V(:,KK)
    enddo
