   ...
   if (wlc_nPointsMoved > 0) then
      ...
      if (netSterics) then
         ...
         do j = 1, wlc_nPointsMoved
            ...
            if ((ANY(ignore == i - 1) .eqv. .false.) .AND. (i > first_bead_of_chain(IP))) then
               ignore(k) = i - 1
               k = k + 1
               call find_neighbors(RGJK(:, i - 1), 2*WLC_P__GJK_RADIUS, &
            ...
            if ((ANY(ignore == i) .eqv. .false.) .AND. (i < last_bead_of_chain(IP))) then
               ignore(k) = i
               k = k + 1
               call find_neighbors(RGJK(:, i), 2*WLC_P__GJK_RADIUS, RGJK, &
                                   WLC_P__NT, WLC_P__NT, neighbors, distances, nn)
               ! check for collisions
      ...
      do j = 1, wlc_nPointsMoved
         ...
         if ((ANY(ignore == i - 1) .eqv. .false.) .AND. (i > first_bead_of_chain(IP))) then
            ignore(k) = i - 1
            k = k + 1
            call find_neighbors(RGJK(:, i - 1), 2*WLC_P__GJK_RADIUS, RGJK, WLC_P__NT, &
         ...
         if ((ANY(ignore == i) .eqv. .false.) .AND. (i < last_bead_of_chain(IP))) then
            ignore(k) = i
            k = k + 1
            call find_neighbors(RGJK(:, i), 2*WLC_P__GJK_RADIUS, RGJK, WLC_P__NT, &
                                WLC_P__NT, neighbors, distances, nn)
            ! check for collisions
