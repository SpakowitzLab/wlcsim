...
!   writhe and twist after each move which is necessary 
!   to ensure that linking number is constant, and 
!   there is no self-crossing.
!
...
! Calculate linking number of the whole ring by Tw + Wr from scratch
...
! Calculate change in Tw, Wr, Lk
subroutine calcDelTwWrLk(IB1, IB2, MCTYPE, delTw, delWr, delLk)
implicit none
...
if (MCTYPE == 4) then
    ...
    delWr = getDelWr(IB1, IB2)
endif
delLk = delTw + delWr

end subroutine 

! Calculate change in linking number after a move by delTw + delWr
function getDelLk(IB1, IB2) result(delLk)
implicit none
integer, intent(in) :: IB1
integer, intent(in) :: IB2
...
! Calculate change in writhe after a move using Brad's WRITHE_MOVE
! This function exists for further development in the case that
! different writhe calculation methods will be used.
function getDelWr(IB1, IB2) result(delWr)
use params, only: wlc_R, wlc_RP
implicit none
real(dp) delWr      ! result change in writhe
integer, intent(in) :: IB1
integer, intent(in) :: IB2

delWr = delWrithe_onlyMoved(IB1, IB2, WLC_P__NB)

end function

! Test delLk with total chain calculation
...
! Test delTw with calculating total twist
...
! and after the move. This function serves
...
! Calculate change in writhe by evaluating the integrand that is changed of the
! double integral described in Klenin (2000) Method 1 before and after a move
function delWrithe_onlyMoved(IB1, IB2, N) result(delWr)
...
! To iterate through every changed integrand (Omega), we nead to pair the inner
! segments up among themselves, and pair each inner segment with all outer segments
! since writhe calculation is the double integral of Omega.
...
! This is a helper function of delWrithe_onlyMoved.
...
if (I1P1 == I2 .or. I2P1 == I1) then
    ...
    Omega = dWrithe_Klenin1b(wlc_R(:, I1), wlc_R(:, I1P1), wlc_R(:, I2), wlc_R(:, I2P1))
...
!    print*, 'RPI1', RPI1, 'RPI1P1', RPI1P1
!    print*, 'RPI2', RPI2, 'RPI2P1', RPI2P1
...
! This function find the value  of the integrand for each pair of chain segment using 
! Method 1b described  in Klenin (2000)
...
!print*, 'Omega in dWrithe_Klenin1b', Omega
