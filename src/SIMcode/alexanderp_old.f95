...
! This is an old version of the code. A newer version has been written that has improved performance

SUBROUTINE ALEXANDERP_OLD(R,N,DELTA)
  ...
  DOUBLE PRECISION, ALLOCATABLE ::  Cross(:,:)        ! matrix of cross indices and coordinates
  ...
  DOUBLE PRECISION theti,thetaj ! angle between real segment and projected segment
  DOUBLE PRECISION, ALLOCATABLE :: over_ind(:) !Vector of over_pass indices (index of over-pass of Nth crossing)
  ...
  !Allocate matrix containing information on crossings.
  !Over-allocate the matrix size to avoid having to resize matrix
  ALLOCATE(Cross(N+100,6))
  ...
  DO I=1,N
     ...
     DO J=1,N
        ...
        !If the intersection point is within length of segments, count as an intersection
         IF (sint.GE.0.AND.sint.LT.smax.AND.tint.GE.0.AND.tint.LT.tmax) THEN
            ...
            !If this is an undercrossing, store it in cross
     ...
     ENDDO
     !After summing over j, set Ndegen for all instances of crossings for segment i
     Cross(Ncross-Ndegen+1:Ncross,5)=Ndegen
  ENDDO
...
!Sort under-crossings of same segment with respect to order of occurrence (w.r.t sint)
  ...
  DO I=1,Ncross
     ...
     IF (J.LT.Cross(1,1)) THEN
        over_ind(I)=1
     ELSEIF (J.GT.Cross(Ncross,1)) THEN
        over_ind(I)=Ncross+1
     ENDIF
