...
def combine(subsumed_section_id,subsuming_section_id,sections,membership):
    ...
    left = min(subsumed['L_slide_bead'],subsuming['L_slide_bead'])
    right = max(subsumed['R_slide_bead'],subsuming['R_slide_bead'])
...
def print_section(section):
    print(str(section['L_toe'])+'-'+str(section['L_knee'])+'-'
...
def growLeg(section_id,bindpairs,membership,sections,leglength=10,moveable_loop=50,direction=1):
    ...
    for ii in range(0,leglength):
        ...
        collision = checkForCollision(knee,membership,section_id,slide_section,sections)
        if collision:
            return collision
    ...
    for ii in range(0,leglength):
        ...
        collision = checkForCollision(toe,membership,section_id,slide_section,sections)
        if collision:
            return collision
